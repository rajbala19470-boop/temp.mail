...
def init_db():
    ...
    """)
    # Check for missing columns (ensure total_emails exists)
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_userlist":
        ...
        display_name = username or first_name or str(uid)
        # No ban status to show
        text = f"User: {display_name}\nID: {uid}\nEmails created: {total_emails}"
        # No ban/unban buttons, just a back button
...
async def broadcast_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = update.message.text
    users = get_all_users()  # each user is (uid, username, first_name, total_emails)
    success = 0
    failed = 0
